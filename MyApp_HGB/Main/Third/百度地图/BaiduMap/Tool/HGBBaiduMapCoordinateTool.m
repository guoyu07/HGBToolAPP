...
@implementation HGBBaiduMapCoordinateTool
...
+(double)distanceBetweenOrderByCurrentLocationLatitude:(double)lat1 Longitude:(double)lng1 andWithOtherLocationLatitude:(double)lat2 Longitude:(double)lng2{
    ...
    currnetCoordinate=[HGBBaiduMapCoordinateTool transformFromGCJToWGS:currnetCoordinate];
...
+(double)distanceBetweenOrderByCurrentCoordinate2D:(CLLocationCoordinate2D)curCoordinate ToOtherCoordinate2D:(CLLocationCoordinate2D)otherCoordinate{
    return [HGBBaiduMapCoordinateTool distanceBetweenOrderByCurrentLocationLatitude:curCoordinate.latitude Longitude:curCoordinate.longitude andWithOtherLocationLatitude:otherCoordinate.latitude Longitude:otherCoordinate.longitude];
}
#pragma mark 地理位置纠错
...

{
    ...
    if([HGBBaiduMapCoordinateTool isLocationOutOfChina:wgsLoc])
...

    {
        ...
        double adjustLat = [HGBBaiduMapCoordinateTool transformLatWithX:wgsLoc.longitude - 105.0 withY:wgsLoc.latitude - 35.0];

        double adjustLon = [HGBBaiduMapCoordinateTool transformLonWithX:wgsLoc.longitude - 105.0 withY:wgsLoc.latitude - 35.0];
...

{
...

    {
        ...
        CLLocationCoordinate2D leftBottom= [HGBBaiduMapCoordinateTool transformFromWGSToGCJ:(CLLocationCoordinate2D){.latitude = minLat,.longitude=minLng}];

        CLLocationCoordinate2D rightBottom = [HGBBaiduMapCoordinateTool transformFromWGSToGCJ:(CLLocationCoordinate2D){.latitude = minLat,.longitude = maxLng}];

        CLLocationCoordinate2D leftUp= [HGBBaiduMapCoordinateTool transformFromWGSToGCJ:(CLLocationCoordinate2D){.latitude = maxLat,.longitude = minLng}];

        CLLocationCoordinate2D midPoint= [HGBBaiduMapCoordinateTool transformFromWGSToGCJ:(CLLocationCoordinate2D){.latitude = ((minLat + maxLat) / 2),.longitude = ((minLng + maxLng) / 2)}];
