...
@interface HGBAnalyticsViewController ()
/**
 数据源
 */
...
@implementation HGBAnalyticsViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"友盟统计:":@[@"事件1",@"事件2"]};
    self.keys=@[@"友盟统计:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [MobClick event:UMAnalytics_Event1];//活动报名友盟统计
        }else if (indexPath.row==1){
            [MobClick event:UMAnalytics_Event2];//活动报名友盟统计
        }
    }
}
