...
#import "HGBThirdMapViewController.h"
#import "HGBAnalyticsViewController.h"
#import "HGBShareViewController.h"

#define Identify_Cell @"cell"
@interface HGBThirdViewController ()
/**
 数据源
 */
...
@implementation HGBThirdViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"常用第三方:":@[@"第三方地图",@"统计",@"分享"]};
    self.keys=@[@"常用第三方:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [self presentController:[[HGBThirdMapViewController alloc]init]];
        }else if (indexPath.row==1){
            [self presentController:[[HGBAnalyticsViewController alloc]init]];
        }else if (indexPath.row==2){
            [self presentController:[[HGBShareViewController alloc]init]];
