...
//
//  HGBApplicationBageViewController.m
//  MyApp_HGB
//
...
@interface HGBApplicationBageViewController ()
/**
 数据源
 */
...
@implementation HGBApplicationBageViewController
...
- (void)viewDidLoad {
    ...
    [self viewSetUp];//UI
    [HGBComponentTool setTabBarBadgeColor:[UIColor yellowColor] inController:self];
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"application角标:":@[@"角标设置为1",@"角标+1",@"角标-1",@"角标隐藏"]};
    self.keys=@[@"application角标:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [HGBComponentTool setApplicationBadge:1];
            NSLog(@"设置成功");
        }else if (indexPath.row==1){
            [HGBComponentTool addApplicationBadge];
            NSLog(@"设置成功");
        }else if (indexPath.row==2){
            [HGBComponentTool reduceApplicationBadge];
            NSLog(@"设置成功");
        }else if (indexPath.row==3){
            [HGBComponentTool hideApplicationBadge];
            NSLog(@"设置成功");
        }
    }
}
