...
@interface HGBTabBarBage1Controller ()
/**
 数据源
 */
...
@implementation HGBTabBarBage1Controller
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"tabBar角标:":@[@"角标设置为1",@"角标+1",@"角标-1",@"角标隐藏"]};
    self.keys=@[@"tabBar角标:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [HGBComponentTool setTabBarBadge:@"1" inController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==1){
            [HGBComponentTool addTabBarBadgeInController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==2){
            [HGBComponentTool reduceTabBarBadgeInController:self];
            NSLog(@"设置成功");
