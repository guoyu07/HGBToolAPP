...
@interface HGBTabBarBage3Controller ()
/**
 数据源
 */
...
@implementation HGBTabBarBage3Controller
...
- (void)viewDidLoad {
    ...
    [self viewSetUp];//UI
     [HGBComponentTool setTabBarBadgeColor:[UIColor redColor] inController:self];
}
#pragma mark 导航栏
//导航栏
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"tabBar标记点:":@[@"标记点设置颜色",@"标记点显示",@"标记点隐藏"]};
    self.keys=@[@"tabBar标记点:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [HGBComponentTool setTabBarPointColor:[UIColor blueColor] inController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==1){
            [HGBComponentTool addPointInController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==2){
            [HGBComponentTool hideTabBarPointInController:self];
            NSLog(@"设置成功");
        }
    }
}
