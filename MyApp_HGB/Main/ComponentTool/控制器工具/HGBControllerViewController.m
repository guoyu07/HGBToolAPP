...
#import "HGBPP1ViewController.h"
#import "HGBPD1ViewController.h"
#import "HNavViewController.h"

@interface HGBControllerViewController ()
/**
 数据源
 */
...
@implementation HGBControllerViewController
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"控制器工具:":@[@"跳转Pop-Push",@"跳转Present-Dismiss",@"导航栏"]};
    self.keys=@[@"控制器工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            ...
            [self presentController:[[HGBPP1ViewController alloc]init]];
        }else if (indexPath.row==1){
             [self presentController:[[HGBPD1ViewController alloc]init]];

        }else if (indexPath.row==2){

            [self presentController:[[HNavViewController alloc]init]];
