...
#import "HGBPD6ViewController.h"
#import "HGBCommonSelectCell.h"
#define Identify_Cell @"cell"
#import "UIViewController+HGBUntil.h"
#import "HGBPD2ViewController.h"

@interface HGBPD6ViewController ()
/**
 数据源
 */
...
@implementation HGBPD6ViewController
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"控制器跳转:":@[@"返回上一层控制器",@"返回首层控制器",@"返回上两层控制器",@"返回指定控制器"]};
    self.keys=@[@"控制器跳转:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [self dismissToParentViewController];
        }else if (indexPath.row==1){
            [self dismissToRootViewController];

        }else if (indexPath.row==2){
            [self dismissToControllerWithDismissCount:2];

        }else if (indexPath.row==3){
            [self dismissToControllerWithControllerName:@"HGBPD2ViewController"];
