...
@interface HGBPP6ViewController ()
/**
 数据源
 */
...
@implementation HGBPP6ViewController
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [self popToParentViewController];
        }else if (indexPath.row==1){
            [self popToRootViewController];

        }else if (indexPath.row==2){
            [self popToControllerWithPopCount:2];

        }else if (indexPath.row==3){
            [self popToControllerWithControllerName:@"HGBPP2ViewController"];
