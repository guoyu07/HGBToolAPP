//
//  HGBTableSelectorViewController.m
//  MyApp_HGB
//
...
@interface HGBTableSelectorViewController ()<HGBTableSelectorDelegate>
/**
 数据源
 */
...
@implementation HGBTableSelectorViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"表格选择:":@[@"表格选择1",@"表格选择2"]};
    self.keys=@[@"表格选择:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            HGBTableSelector *tableSelector=[[HGBTableSelector alloc]init];
            tableSelector.delegate=self;
            tableSelector.nameStr=@"name";
            tableSelector.dataSource=@{@"1":@[@{@"name":@"huang",@"id":@"1"},@{@"name":@"zhuang",@"id":@"2"}]};
            [tableSelector createNavigationItemWithTitle:@"表格选择1"];
            UINavigationController *nav=[[UINavigationController alloc]initWithRootViewController:tableSelector];
            [self presentViewController:nav animated:YES completion:nil];
        }else if (indexPath.row==1){
            HGBTableSelector *tableSelector=[[HGBTableSelector alloc]init];
            tableSelector.delegate=self;
            tableSelector.nameStr=@"name";
           [tableSelector createNavigationItemWithTitle:@"表格选择2"]; tableSelector.dataSource=@{@"1":@[@{@"name":@"huang",@"id":@"1"},@{@"name":@"zhuang",@"id":@"2"}],@"2":@[@{@"name":@"liu",@"id":@"3"},@{@"name":@"li",@"id":@"4"}]};
            UINavigationController *nav=[[UINavigationController alloc]initWithRootViewController:tableSelector];
            [self presentViewController:nav animated:YES completion:nil];
        }
