...
@interface HGBCameraViewController ()<HGBCommonCameraControllerDelegate>
/**
 数据源
 */
...
@implementation HGBCameraViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"自定义相机:":@[@"自定义相机"]};
    self.keys=@[@"自定义相机:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            HGBCommonCameraController *cameraVC=[[HGBCommonCameraController alloc]init];
