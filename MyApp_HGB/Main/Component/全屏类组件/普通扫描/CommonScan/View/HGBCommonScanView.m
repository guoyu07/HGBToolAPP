...
@interface HGBCommonScanView()

/**
 扫码区域各种参数
 */
@property (nonatomic, strong) HGBCommonScanViewStyle* viewStyle;
...
/**
 线条扫码动画封装
 */
@property (nonatomic,strong)HGBCommonScanLineAnimation *scanLineAnimation;
/**
 网格扫码动画封装
 */
@property (nonatomic,strong)HGBCommonScanNetAnimation *scanNetAnimation;
...
@end
@implementation HGBCommonScanView
...
- (void)startScanAnimation
{
    ...
    {
        case HGBCommonScanViewAnimationStyle_LineMove:
        {
            //线动画
            if (!_scanLineAnimation)
            self.scanLineAnimation = [[HGBCommonScanLineAnimation alloc]init];
            [_scanLineAnimation startAnimatingWithRect:_scanRetangleRect InView:self Image:_viewStyle.animationImage];
        }
        break;
        case HGBCommonScanViewAnimationStyle_NetGrid:
        {
            //网格动画
            if (!_scanNetAnimation)
            self.scanNetAnimation = [[HGBCommonScanNetAnimation alloc]init];
        ...
        break;
        case HGBCommonScanViewAnimationStyle_LineStill:
        {
            if (!_scanLineStill) {
...
- (void)drawScanRect
{
    ...
    {
        case HGBCommonScanViewPhotoframeAngleStyle_Outer:
        {
            diffAngle = linewidthAngle/3;//框外面4个角，与框紧密联系在一起
        }
        break;
        case HGBCommonScanViewPhotoframeAngleStyle_On:
        {
            diffAngle = 0;
        }
        break;
        case HGBCommonScanViewPhotoframeAngleStyle_Inner:
        {
