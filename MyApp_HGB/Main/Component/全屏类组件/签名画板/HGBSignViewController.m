...
@interface HGBSignViewController ()<HGBSignDrawerDelegate>
/**
 数据源
 */
...
@implementation HGBSignViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            HGBSignDrawer *drawer=[HGBSignDrawer instanceWithParent:self andWithDelegate:self];
            drawer.isSaveToCache=YES;

           [drawer popInParentView];
