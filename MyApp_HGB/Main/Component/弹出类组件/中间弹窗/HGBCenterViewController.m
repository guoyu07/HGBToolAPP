...
@interface HGBCenterViewController ()<HGBCenterPopDelegate>
/**
 数据源
 */
...
@implementation HGBCenterViewController
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            HGBCenterPopItemModel *m0=[[HGBCenterPopItemModel alloc]init];
            m0.promptImage=[UIImage imageNamed:@"cir0"];
            m0.promptTitle=@"选项0";
            m0.idx=@"1";

            HGBCenterPopItemModel *m1=[[HGBCenterPopItemModel alloc]init];
            m1.promptImage=[UIImage imageNamed:@"cir1"];
            m1.idx=@"2";
            m1.promptTitle=@"选项1";


            NSArray *array=@[m0,m1];
            HGBCenterPop *center=[HGBCenterPop instanceWithDelegate:self andWithParent:self];
            center.dataSource=array;
            center.titleStr=@"标题";
            [center popInParentView];
        }
...
-(void)centerPopDidSelectedAtIndex:(NSInteger)idx withCenterModel:(HGBCenterPopItemModel *)centerModel{
    NSLog(@"%ld-%@",idx,centerModel.promptTitle);
}
#pragma mark get
