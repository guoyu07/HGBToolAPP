...
/** 主题容器view */
@property (nonatomic, strong) HGBDropView * topicContainerView;
/** 装载tableView的背景容器 */
...
- (void)setUpUI{
    //主题容器view
    self.topicContainerView = [[HGBDropView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame))];
...
- (void)show{
    ...
    if (self.topicContainerView.isSelected) {
        self.tableView.frame=CGRectMake(self.tableView.frame.origin.x, self.tableView.frame.origin.y, self.tableView.frame.size.width, _heightForRow*self.itemArray.count);
        ...
        self.frame=CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width,self.topicContainerView.frame.size.height+_heightForRow*self.itemArray.count);
        self.tableViewContainerView.frame=CGRectMake(self.tableViewContainerView.frame.origin.x, self.tableViewContainerView.frame.origin.y, self.tableViewContainerView.frame.size.width,_heightForRow*self.itemArray.count);
        self.tableViewContainerView.layer.shadowPath = [UIBezierPath bezierPathWithRect:_tableViewContainerView.bounds].CGPath;
...
- (void)hideOtherDropDown{
    for (UIView * subview in self.superview.subviews) {
        if ([subview isKindOfClass:[HGBDropDown class]]) {
            HGBDropDown * dropDown = (HGBDropDown *)subview;
            if (subview != self) {
...
- (void)setDelegate:(id<HGBDropDownDelegate>)delegate{
    _delegate = delegate;
