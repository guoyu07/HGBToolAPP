...
typedef NS_ENUM(NSUInteger, HGBPageControlImageType) {
    HGBPageControlImageTypeNormal = 1,
    HGBPageControlImageTypeCurrent,
    HGBPageControlImageTypeMask
};

typedef NS_ENUM(NSUInteger, HGBPageControlStyleDefaults) {
    HGBPageControlDefaultStyleClassic = 0,
    HGBPageControlDefaultStyleModern
};

static HGBPageControlStyleDefaults _defaultStyleForSystemVersion;

@interface HGBPageControl ()
...
+ (void)initialize
{
    ...
    if ([currSysVer compare:reqSysVer options:NSNumericSearch] != NSOrderedAscending) {
        _defaultStyleForSystemVersion = HGBPageControlDefaultStyleModern;
    } else {
        _defaultStyleForSystemVersion = HGBPageControlDefaultStyleClassic;
    }
}
...
{
    _numberOfPages = 0;
    _tapBehavior = HGBPageControlTapBehaviorStep;
    ...
    _alignment = HGBPageControlAlignmentCenter;
    _verticalAlignment = HGBPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
    ...
    CGFloat left = 0.0f;
    switch (_alignment) {
        case HGBPageControlAlignmentCenter:
            left = ceilf(CGRectGetMidX(rect) - (size.width / 2.0f));
            break;
        case HGBPageControlAlignmentRight:
...
- (CGFloat)_topOffsetForHeight:(CGFloat)height rect:(CGRect)rect
{
    ...
    switch (_verticalAlignment) {
        case HGBPageControlVerticalAlignmentMiddle:
            top = CGRectGetMidY(rect) - (height / 2.0f);
            break;
        case HGBPageControlVerticalAlignmentBottom:
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex type:(HGBPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return;
    }
    ...
    switch (type) {
        case HGBPageControlImageTypeCurrent:
            dictionary = self.currentPageImages;
            break;
        case HGBPageControlImageTypeNormal:
            dictionary = self.pageImages;
            break;
        case HGBPageControlImageTypeMask:
...
{
    [self _setImage:image forPage:pageIndex type:HGBPageControlImageTypeNormal];
    [self _updateMeasuredIndicatorSizes];
}

- (void)setCurrentImage:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:HGBPageControlImageTypeCurrent];;
    [self _updateMeasuredIndicatorSizes];
}

- (void)setImageMask:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:HGBPageControlImageTypeMask];
...
- (id)_imageForPage:(NSInteger)pageIndex type:(HGBPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return nil;
    }
...
{
    return [self _imageForPage:pageIndex type:HGBPageControlImageTypeNormal];
}

- (UIImage *)currentImageForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:HGBPageControlImageTypeCurrent];
}

- (UIImage *)imageMaskForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:HGBPageControlImageTypeMask];
}
...
- (void)setStyleWithDefaults:(HGBPageControlStyleDefaults)defaultStyle
{
    switch (defaultStyle) {
        case HGBPageControlDefaultStyleModern:
            ...
            break;
        case HGBPageControlDefaultStyleClassic:
        default:
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (HGBPageControlTapBehaviorJump == self.tapBehavior) {
