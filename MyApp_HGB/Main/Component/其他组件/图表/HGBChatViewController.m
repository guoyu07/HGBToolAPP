...
@interface HGBChatViewController ()
/**
 数据源
 */
...
@implementation HGBChatViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"图表:":@[@"条形图",@"折线图",@"饼状图"]};
    self.keys=@[@"图表:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [self presentController:[[HGBBarChatViewController alloc]init] ];
        }else if (indexPath.row==1){
             [self presentController:[[HGBBreakPointLineChatViewController alloc]init] ];
        }else if (indexPath.row==2){
             [self presentController:[[HGBPieChatViewController alloc]init] ];
        }
    }
}
#pragma mark 控制器跳转
