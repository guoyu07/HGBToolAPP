//
//  HGBCirculateScrollPageIControl.m
//  测试
//
...
typedef NS_ENUM(NSUInteger, HGBCirculateScrollPageControlImageType) {
    HGBCirculateScrollPageControlImageTypeNormal = 1,
    HGBCirculateScrollPageControlImageTypeCurrent,
    HGBCirculateScrollPageControlImageTypeMask
};

typedef NS_ENUM(NSUInteger, HGBCirculateScrollPageControlStyleDefaults) {
    HGBCirculateScrollPageControlDefaultStyleClassic = 0,
    HGBCirculateScrollPageControlDefaultStyleModern
};

static HGBCirculateScrollPageControlStyleDefaults _defaultStyleForSystemVersion;

@interface HGBCirculateScrollPageIControl ()
...
@end
@implementation HGBCirculateScrollPageIControl
...
+ (void)initialize
{
    ...
    if ([currSysVer compare:reqSysVer options:NSNumericSearch] != NSOrderedAscending) {
        _defaultStyleForSystemVersion = HGBCirculateScrollPageControlDefaultStyleModern;
    } else {
        _defaultStyleForSystemVersion = HGBCirculateScrollPageControlDefaultStyleClassic;
    }
}
...
{
    _numberOfPages = 0;
    _tapBehavior = HGBCirculateScrollPageControlTapBehaviorStep;
    ...
    _alignment = HGBCirculateScrollPageControlAlignmentCenter;
    _verticalAlignment = HGBCirculateScrollPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
    ...
    CGFloat left = 0.0f;
    switch (_alignment) {
        case HGBCirculateScrollPageControlAlignmentCenter:
        left = ceilf(CGRectGetMidX(rect) - (size.width / 2.0f));
        break;
        case HGBCirculateScrollPageControlAlignmentRight:
...
- (CGFloat)_topOffsetForHeight:(CGFloat)height rect:(CGRect)rect
{
    ...
    switch (_verticalAlignment) {
        case HGBCirculateScrollPageControlVerticalAlignmentMiddle:
        top = CGRectGetMidY(rect) - (height / 2.0f);
        break;
        case HGBCirculateScrollPageControlVerticalAlignmentBottom:
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex type:(HGBCirculateScrollPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return;
    }
    ...
    switch (type) {
        case HGBCirculateScrollPageControlImageTypeCurrent:
        dictionary = self.currentPageImages;
        break;
        case HGBCirculateScrollPageControlImageTypeNormal:
        dictionary = self.pageImages;
        break;
        case HGBCirculateScrollPageControlImageTypeMask:
...
{
    [self _setImage:image forPage:pageIndex type:HGBCirculateScrollPageControlImageTypeNormal];
    [self _updateMeasuredIndicatorSizes];
}

- (void)setCurrentImage:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:HGBCirculateScrollPageControlImageTypeCurrent];;
    [self _updateMeasuredIndicatorSizes];
}

- (void)setImageMask:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:HGBCirculateScrollPageControlImageTypeMask];
...
- (id)_imageForPage:(NSInteger)pageIndex type:(HGBCirculateScrollPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return nil;
    }
...
{
    return [self _imageForPage:pageIndex type:HGBCirculateScrollPageControlImageTypeNormal];
}

- (UIImage *)currentImageForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:HGBCirculateScrollPageControlImageTypeCurrent];
}

- (UIImage *)imageMaskForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:HGBCirculateScrollPageControlImageTypeMask];
}
...
- (void)setStyleWithDefaults:(HGBCirculateScrollPageControlStyleDefaults)defaultStyle
{
    switch (defaultStyle) {
        case HGBCirculateScrollPageControlDefaultStyleModern:
        ...
        break;
        case HGBCirculateScrollPageControlDefaultStyleClassic:
        default:
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (HGBCirculateScrollPageControlTapBehaviorJump == self.tapBehavior) {
