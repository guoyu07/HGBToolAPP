//
//  HGBNotificationPopViewController.m
//  MyApp_HGB
//
...
@interface HGBNotificationPopViewController ()<HGBNotificationDelegate>
/**
 数据源
 */
...
@implementation HGBNotificationPopViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
    [self createNavigationItem];//导航栏
    [self viewSetUp];//UI
    [HGBNotification setDelegate:self];
    [HGBNotification setBackgroundNotiEnable:NO];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"消息弹窗:":@[@"消息弹窗",@"消息弹窗颜色"]};
    self.keys=@[@"消息弹窗:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [HGBNotification hgb_notificationWithTitle:@"hello" msg:@"hello world"];
        }else if (indexPath.row==1){
            [HGBNotification setMsgBackGroundColor:[UIColor yellowColor]];
            [HGBNotification setTitleBackGroundColor:[UIColor greenColor]];
            [HGBNotification hgb_notificationWithTitle:@"hello" msg:@"hello world"];
        }
    }
}
