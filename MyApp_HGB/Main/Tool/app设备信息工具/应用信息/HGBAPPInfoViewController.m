...
@interface HGBAPPInfoViewController ()
/**
 数据源
 */
...
@implementation HGBAPPInfoViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"应用信息(控制台打印):":@[@"app版本号",@"app Build版本号",@"app名称",@"app bundle id",@"app图标",@"app infoPlist",@"app启动图标"]};
    self.keys=@[@"应用信息(控制台打印):"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            ...
            NSLog(@"%@",[HGBAPPInfoTool getLocalAppVersion]);
        }else if (indexPath.row==1){
            NSLog(@"%@",[HGBAPPInfoTool getLocalAppBuildVersion]);

        }else if (indexPath.row==2){
            NSLog(@"%@",[HGBAPPInfoTool getAppName]);

        }else if (indexPath.row==3){
            NSLog(@"%@",[HGBAPPInfoTool getBundleID]);

        }else if (indexPath.row==4){
            NSLog(@"%@",[HGBAPPInfoTool getAppImage]);

        }else if (indexPath.row==5){
            NSLog(@"%@",[HGBAPPInfoTool getInfoPlist]);

        }else if (indexPath.row==6){
            NSLog(@"%@",[HGBAPPInfoTool getLanuchImage]);
