...
+ (NSString *)getAllIPAddress:(BOOL)preferIPv4
{
    ...
    if(preferIPv4){
        address=addresses[@"en0/ipv4"];
        if(address==nil||address.length<4){
            address=addresses[@"pdp_ip0/ipv4"];
        }
    }else{
...
+ (NSString *)getIPAddressWithIpType:(NSString *)type{
    ...
    NSDictionary *ips=[HGBIPTool getAllIPAddresses];
    if(ips==nil||ips.allKeys.count==0){
        return nil;
    }
    return [ips objectForKey:type];
}
/**
 获取所有的IP
