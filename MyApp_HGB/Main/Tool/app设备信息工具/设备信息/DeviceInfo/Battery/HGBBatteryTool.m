...
@interface HGBBatteryTool ()
@property (nonatomic, assign) BOOL batteryMonitoringEnabled;

@end

@implementation HGBBatteryTool

+ (instancetype)sharedInstance {
    static HGBBatteryTool *tool;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        tool = [[HGBBatteryTool alloc] init];
    });
    return tool;
}
...
+(NSInteger)getBatteryCapacity {
    return BatteryCapacityContainer[[HGBBatteryTool transformMachineToIdevice]];
}

+ (CGFloat)getBatterVolocity {
    return BatteryVoltageContainer[[HGBBatteryTool transformMachineToIdevice]];
}
#pragma mark - Setters && Getters
- (CGFloat)voltage {
    return [HGBBatteryTool getBatterVolocity];
}

- (NSUInteger)capacity {
    return [HGBBatteryTool getBatteryCapacity];
}
