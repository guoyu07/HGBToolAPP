 ...
 */
@interface HGBThreadViewController ()
/**
 数据源
 */
...
@implementation HGBThreadViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"线程工具请查看控制版打印:":@[@"主线程",@"顺序执行",@"异步并发"]};
    self.keys=@[@"线程工具请查看控制版打印:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [HGBThreadTool performActionOnMainThread:^{
                NSLog(@"%@",[NSThread currentThread]);
            }];
        }else if (indexPath.row==1){
            [HGBThreadTool performActionsInOrder:@[^{
                NSLog(@"%@",[NSThread currentThread]);
            },^{
                NSLog(@"%@",[NSThread currentThread]);
            },^{
                NSLog(@"%@",[NSThread currentThread]);
            }] OnThread:[HGBThreadTool getGlobalThread]];
        }else if (indexPath.row==2){
            [HGBThreadTool performActionsConCurrentAsync:@[^{
                NSLog(@"%@",[NSThread currentThread]);
            },^{
                NSLog(@"%@",[NSThread currentThread]);
            },^{
                NSLog(@"%@",[NSThread currentThread]);
            }] onThreadWithThreadFlag:@"黄光宝"];
        }
