...
@interface HGBPredicateViewController ()
/**
 数据源
 */
...
@implementation HGBPredicateViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"谓词(正则表达式)工具请查看控制版打印:":@[@"手机号码校验"]};
    self.keys=@[@"谓词(正则表达式)工具请查看控制版打印:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
           NSLog(@"%d",[HGBPredicateTool evaluateString:@"18811205033" WithCondition:@"^(1[3|4|5|7|8][0-9])\\d{8}$"]);
        }
