...
//
//  HGBStringViewController.m
//  MyApp_HGB
//
//  Created by huangguangbao on 2017/9/25.
...
@interface HGBStringViewController ()
/**
 数据源
 */
...
@implementation HGBStringViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"字符串工具:(由于方法太错仅介绍数种:)":@[@"字符串去两端空格",@"字符串去除所有空格",@"是否是身份证号码"]};
    self.keys=@[@"字符串工具:(由于方法太错仅介绍数种:)"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            ...
            NSLog(@"--%@---",[@" df  sdf " deleteSpace]);
        }else if(indexPath.row==2){
            NSLog(@"%d",[@"121121187607126512" isStritIDCardNum]);
        }
