...
@interface HGBCoordinateViewController ()
/**
 数据源
 */
...
@implementation HGBCoordinateViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"经纬度工具:":@[@"纠错",@"测距",@"获取地址信息"]};
    self.keys=@[@"经纬度工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            CLLocationCoordinate2D coordinate=CLLocationCoordinate2DMake(36.7, 161.12);
            CLLocationCoordinate2D coord= [HGBCoordinateTool transformFromGCJToWGS:coordinate];
            NSLog(@"---%f---%f",coord.latitude,coord.longitude);
        }else if(indexPath.row==1){
            CLLocationCoordinate2D coordinate1=CLLocationCoordinate2DMake(36.7, 161.12);
            CLLocationCoordinate2D coordinate2=CLLocationCoordinate2DMake(35.7, 131.12);
            CGFloat distance=[HGBCoordinateTool distanceBetweenOrderByCurrentCoordinate2D:coordinate1 ToOtherCoordinate2D:coordinate2];
            NSLog(@"--%f---",distance);
        }else if(indexPath.row==2){
            [HGBGeoPositionTool getLocationInfoWithString:@"北辛村" andWithReslutBlock:^(BOOL status, NSDictionary *info, NSArray *infos, CLPlacemark *mark, NSArray *marks) {
                NSLog(@"%d-%@-%@-%@-%@",status,info,infos,mark,marks);
            }];
        }
