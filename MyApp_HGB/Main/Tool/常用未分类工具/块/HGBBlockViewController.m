...
#import "HGBCustomBlock.h"

@interface HGBBlockViewController ()
/**
 数据源
 */
...
@implementation HGBBlockViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"块(仅调用2个其余不再实例):":@[@"常用块1-bool",@"常用块2-string"]};
    self.keys=@[@"块(仅调用2个其余不再实例):"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [self testFlagBlock:^(BOOL flag) {
                NSLog(@"%d",flag);
            }];
        }else if(indexPath.row==1){
            [self testStringBlock:^(NSString *string) {
                NSLog(@"%@",string);
            }];
...
-(void)testFlagBlock:(CommonFlagBlock )flagBlock{
    flagBlock(YES);
}
-(void)testStringBlock:(CommonStringBlock )stringBlock{
    stringBlock(@"string block test");
}
#pragma mark get
