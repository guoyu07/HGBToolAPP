...
#import "HGBTimeTool.h"

#import "HGBCommonSelectCell.h"
#define Identify_Cell @"cell"


@interface HGBTimeToolViewController ()
/**
 数据源
 */
...
@implementation HGBTimeToolViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"时间工具:":@[@"获取秒级时间搓",@"获取毫秒级时间搓",@"获取标准时间字符串",@"获取网络时间",@"获取网络时间字符串",@"标准时间转字符串",@"标准时间字符串转时间"]};
    self.keys=@[@"时间工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSLog(@"%@",[HGBTimeTool getSecondTimeStringSince1970]);
        }else  if(indexPath.row==1){
            NSLog(@"%@",[HGBTimeTool getMillisecondTimeStringSince1970]);

        }else  if(indexPath.row==2){
            NSLog(@"%@",[HGBTimeTool getFormatTimeString]);

        }else  if(indexPath.row==3){
            NSLog(@"%@",[HGBTimeTool getInternetDate]);

        }else  if(indexPath.row==4){
            NSLog(@"%@",[HGBTimeTool getInternetDateFormatTimeString]);

        }else  if(indexPath.row==5){
            NSLog(@"%@",[HGBTimeTool getFormatDateStringFromDate:[NSDate date]]);

        }else  if(indexPath.row==6){
            NSLog(@"%@",[HGBTimeTool getDateFromFormatDateString:@"19910913081230"]);
