...
@interface HGBFileBaseTableCell()
/**
 短底部分割线
 */
...
@implementation HGBFileBaseTableCell
#pragma mark init
...
{
    //上分割线
    self.topLine=[[UIImageView alloc]initWithFrame: CGRectMake(0,0,kWidth,1)];
    self.topLine.backgroundColor=[UIColor lightGrayColor];
    self.topLine.hidden=YES;
    [self addSubview:self.topLine];

    //下分割线
    self.bottomLine=[[UIImageView alloc]initWithFrame: CGRectMake(0,height-1,kWidth,1)];
    self.bottomLine.backgroundColor=[UIColor lightGrayColor];
    self.bottomLine.hidden=YES;
    [self addSubview:self.bottomLine];

    //短分割线
    self.line=[[UIImageView alloc]initWithFrame: CGRectMake(30*wScale,height-1,kWidth-30*wScale,1)];
