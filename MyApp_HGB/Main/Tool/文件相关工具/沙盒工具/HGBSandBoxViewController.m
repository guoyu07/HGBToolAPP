...
@interface HGBSandBoxViewController ()
/**
 数据源
 */
...
@implementation HGBSandBoxViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"沙盒工具":@[@"获取沙盒路径",@"获取沙盒Documents路径",@"获取沙盒Library路径",@"获取沙盒Caches路径",@"获取沙盒Preferances路径",@"获取沙盒Tmp路径"]};
    self.keys=@[@"沙盒工具"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSLog(@"%@",[HGBSandBoxTool getHomeFilePath]);
        }else if (indexPath.row==1){
            NSLog(@"%@",[HGBSandBoxTool getDocumentFilePath]);

        }else if (indexPath.row==2){
            NSLog(@"%@",[HGBSandBoxTool getLibraryFilePath]);

        }else if (indexPath.row==3){
            NSLog(@"%@",[HGBSandBoxTool getCacheFilePath]);

        }else if (indexPath.row==4){
            NSLog(@"%@",[HGBSandBoxTool getPreferenceFilePath]);

        }else if (indexPath.row==5){
            NSLog(@"%@",[HGBSandBoxTool getTmpFilePath]);
