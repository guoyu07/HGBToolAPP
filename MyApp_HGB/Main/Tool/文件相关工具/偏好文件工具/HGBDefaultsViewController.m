...
@interface HGBDefaultsViewController ()
/**
 数据源
 */
...
@implementation HGBDefaultsViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"偏好工具":@[@"保存",@"查看",@"删除"]};
    self.keys=@[@"偏好工具"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSLog(@"%d",[HGBDefaultsTool saveDefaultsValue:@"hello" WithKey:@"name-name"]);
        }else if (indexPath.row==1){

            NSLog(@"%@",[HGBDefaultsTool getDefaultsWithKey:@"name-name"]);
        }else if (indexPath.row==2){
            NSLog(@"%d",[HGBDefaultsTool deleteDefaultsWithKey:@"name-name"]);
