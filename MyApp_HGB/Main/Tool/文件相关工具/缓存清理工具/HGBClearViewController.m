...
@interface HGBClearViewController ()
/**
 数据源
 */
...
@implementation HGBClearViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"缓存清除:文件管理查看":@[@"清除caches",@"清除documents",@"清除document下文件夹1"]};
    self.keys=@[@"缓存清除:文件管理查看"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBClearStorageTool clearCacheStrorageWithSubPath:nil];

        }else if (indexPath.row==1){
            [HGBClearStorageTool clearDocumentStrorageWithSubPath:nil];

        }else if (indexPath.row==2){
            [HGBClearStorageTool clearDocumentStrorageWithSubPath:@"1"];
