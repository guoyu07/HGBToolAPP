...
@interface HGBShotScreenViewController ()<UIWebViewDelegate>
/**
 数据源
 */
...
@implementation HGBShotScreenViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"截屏工具:":@[@"普通截屏",@"webview全部截屏",@"清空"]};
    self.keys=@[@"截屏工具:"];
    ...
    UIView *headerView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, kWidth, 240)];
    headerView.backgroundColor=[UIColor lightGrayColor];
    self.tableView.tableFooterView=headerView;


    self.webView=[[UIWebView alloc]initWithFrame:CGRectMake(20, 20, 120, 200)];
    self.webView.delegate=self;
    self.webView.scalesPageToFit=YES;

    [headerView addSubview:self.webView];
    NSURL *loadurl;
    loadurl=[NSURL URLWithString:[NSString stringWithFormat:@"https://www.baidu.com.cn"]];
    NSURLRequest *request = [NSURLRequest requestWithURL:loadurl];
    [self.webView loadRequest:request];

    self.imageView=[[UIImageView alloc]initWithFrame:CGRectMake(160, 20, 120, 200)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            self.imageView.image=[HGBScreenShotTool shotCurrentScreenWithView:self.webView];
        }else if (indexPath.row==1){
            self.imageView.image=[HGBScreenShotTool shotFullScreenWithView:self.webView];
