...
@interface HGBSizeViewController ()
/**
 数据源
 */
...
@implementation HGBSizeViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"尺寸工具:":@[@"位置大小",@"文字适应",@"还原"]};
    self.keys=@[@"尺寸工具:"];
    ...
    self.label=[[UILabel alloc]initWithFrame:CGRectMake(10, 20, 300, 17)];
    self.label.backgroundColor=[UIColor yellowColor];
    self.label.text=@"saubfuisdbfchsdb chjdsb c sc bdsjxcfhsdbchbdsjhcvsefhyefbyaesfyudsfcsdbfvuhsdbvchjbsdvcbrlrfsdaljdfldsjbfvfhjdsbvjfsdbvhjsbdhjvbsdhjvbhjdsbvhdsbj";
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            ...
            CGFloat height=[HGBSizeTool heightForString:self.label.text fontSize:15 andWidth:300];
            self.label.frame=CGRectMake(10, 10, 300, height);

        }else if (indexPath.row==2){
            self.label.frame=CGRectMake(10, 20, 300, 17);
        }
