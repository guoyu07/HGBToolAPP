...
#import "UIColor+HGBHexStringColor.h"
#import "HGBColorConfig.h"

#import "HGBCommonSelectCell.h"
#define Identify_Cell @"cell"

@interface HGBColorViewController ()
/**
 数据源
 */
...
@implementation HGBColorViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"颜色工具:":@[@"16进制字符串颜色",@"常用颜色实例",@"颜色宏"]};
    self.keys=@[@"颜色工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            self.tableView.backgroundColor=[UIColor ColorWithHexString:@"#23E745"];
        }else if (indexPath.row==1){
            self.tableView.backgroundColor=HGBRandomColor;
        }else if (indexPath.row==2){
            self.tableView.backgroundColor= HGBColor(0.1, 0.1, 0.1);
