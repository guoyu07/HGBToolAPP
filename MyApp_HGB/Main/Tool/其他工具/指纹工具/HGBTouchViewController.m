...
/**
 ...
 UIKit.framework
 */
@interface HGBTouchViewController ()
/**
 数据源
 */
...
@implementation HGBTouchViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"指纹(TouchID)工具结果在控制台打印:":@[@"指纹识别"]};
    self.keys=@[@"指纹(TouchID)工具结果在控制台打印:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBTouchIDTool touchIDWithMessage:@"指纹识别" cancelTitle:@"取消" otherTitle:@"其他方式解锁" enabled:YES andWithOperatingrResultBlock:^(HGBOperatingTouchIDResult result, NSDictionary *messageInfo) {
                NSLog(@"%@-%@-%@",messageInfo[@"reslutCode"],messageInfo[@"reslutMessage"],messageInfo[@"ReslutError"]);
            }];
        }
