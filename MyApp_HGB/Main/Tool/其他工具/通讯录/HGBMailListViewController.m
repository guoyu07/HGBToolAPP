...
@interface HGBMailListViewController ()<HGBMailListToolDelegate>
/**
 数据源
 */
...
@implementation HGBMailListViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"通讯录":@[@"获取一条通讯录信息",@"获取所有通讯录信息",@"创建联系人"]};
    self.keys=@[@"通讯录"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBMailListTool openMailListBookInParent:self andWithDelegate:self];

        }else if (indexPath.row==1){
            [HGBMailListTool getMailListWithDelegate:self];
        }else if (indexPath.row==2){
            [HGBMailListTool creatItemWithName:@"" phone:@"18812345678" andWithDelegate:self];
        }
