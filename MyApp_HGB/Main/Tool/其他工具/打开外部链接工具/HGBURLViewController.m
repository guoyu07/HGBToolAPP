...
/**
 调用:1.HGBOpenExternalUrlTool 扫描界面

 framework:
 UIKit.framework

 功能:可打开外部app 浏览器  设置界面 及打电话
 */
@interface HGBURLViewController ()
/**
 数据源
 */
...
@implementation HGBURLViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"url工具:":@[@"打开百度网页",@"打开设置",@"打开微信"]};
    self.keys=@[@"url工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [HGBOpenExternalUrlTool openBrowserWithUrlString:@"http://www.baidu.com"];
        }else if (indexPath.row==1){
            [HGBOpenExternalUrlTool openAppSetView];
        }else if (indexPath.row==2){
            [HGBOpenExternalUrlTool openAppWithUrlString:@"weixin://"];
        }
