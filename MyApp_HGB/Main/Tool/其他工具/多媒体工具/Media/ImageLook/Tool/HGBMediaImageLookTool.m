...
+(void)lookFileAtPath:(NSString *)path inParent:(UIViewController *)parent{
    [HGBMediaImageLookTool shareInstance];
    if(parent==nil){
        [HGBMediaImageLookTool alertWithPrompt:@"parent不能为空"];
    ...
    if(path==nil&&path.length==0){
        [HGBMediaImageLookTool alertWithPrompt:@"路径不能为空"];
        if (instance.delegate&&[instance.delegate respondsToSelector:@selector(imageLookDidOpenFailed:)]) {
            [instance.delegate imageLookDidOpenFailed:instance];
        }
        return;
    }
    [HGBMediaImageLookTool lookFileAtUrl:[[NSURL fileURLWithPath:path] absoluteString] inParent:parent];
}
...
+(void)lookFileAtUrl:(NSString *)url inParent:(UIViewController *)parent{
    ...
    if([url containsString:@"http"]||[url containsString:@"https"]){
        ...
        path=[HGBMediaImageLookTool getCompletePathFromSimplifyFilePath:path];
        url=[[NSURL fileURLWithPath:path]absoluteString];
        if(![HGBMediaImageLookTool isExitAtFilePath:path]){
            [HGBMediaImageLookTool alertWithPrompt:@"文件不存在"];
            if (instance.delegate&&[instance.delegate respondsToSelector:@selector(imageLookDidOpenFailed:)]) {
                [instance.delegate imageLookDidOpenFailed:instance];
            }
            return;
...
+(NSString *)getCompletePathFromSimplifyFilePath:(NSString *)simplifyFilePath{
    ...
    if(![HGBMediaImageLookTool isExitAtFilePath:path]){
        path=[[HGBMediaImageLookTool getHomeFilePath] stringByAppendingPathComponent:simplifyFilePath];
        if(![HGBMediaImageLookTool isExitAtFilePath:path]){
            path=[[HGBMediaImageLookTool getDocumentFilePath] stringByAppendingPathComponent:simplifyFilePath];
            if(![HGBMediaImageLookTool isExitAtFilePath:path]){
                path=[[HGBMediaImageLookTool getMainBundlePath] stringByAppendingPathComponent:simplifyFilePath];
                if(![HGBMediaImageLookTool isExitAtFilePath:path]){
