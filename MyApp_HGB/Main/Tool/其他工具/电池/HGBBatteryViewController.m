...
@interface HGBBatteryViewController ()<HGBBatteryManagerDelegate>
/**
 数据源
 */
...
@implementation HGBBatteryViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"电池信息":@[@"获取电池信息"]};
    self.keys=@[@"电池信息"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
       ...
       if (indexPath.row==0){
            [HGBBatteryManager sharedManager].delegate=self;
            [[HGBBatteryManager sharedManager]startBatteryMonitoring];
