...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"通信工具:":@[@"拨打电话",@"发送短信",@"发送邮件"]};
    self.keys=@[@"通信工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [HGBSystemContactTool callPhone:@"18811025035"];
        }else if (indexPath.row==1){
            [HGBSystemContactTool sendMessage:@"hello" WithRecivePhoneNums:@[@"18811025035"] withReslutBlock:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            } inParent:self];
        }else if (indexPath.row==2){
            [HGBSystemContactTool sendEmailToRecives:@[@"7112002@qq.com"] andWithCopyRecives:nil andWithSecretCopyRecives:nil andWithSubject:@"hello" andWithBody:@"hello world" andWithFileData:nil andWithFileType:nil andWithFileName:nil withReslutBlock:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            } inParent:self];
        }
